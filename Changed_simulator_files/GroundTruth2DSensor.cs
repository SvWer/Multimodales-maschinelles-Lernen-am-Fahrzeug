...
namespace Simulator.Sensors
{
    ...
    public class GroundTruth2DSensor : SensorBase
    {
        ...
        [SensorParameter]
        [Range(0.01f, 2000.0f)]
        public float MaxDistance = 700.0f;
        ...
        void OnCollider(Collider other)
        {
			...
			//If Object is not active or if Layer not GroundTruth (never called)
            ...
            else if (parent.layer == LayerMask.NameToLayer("TrafficLight"))
            {
                if(other.name.Contains("3D"))
                {
                    return;
                } else{
                    var trafC = parent.GetComponent<SignalLight>();
                    if(trafC.GTID == 0)
                    {
                        uint temp = (uint)UnityEngine.Random.Range(20000,40000);
                        trafC.GTID = temp;
                        id = temp;
                    } 
                    else {
                    ...
                    Debug.Log(parent.transform.Find("GroundTruthBox3D").name);
                    var test = parent.transform.Find("GroundTruthBox3D");
                    BoxCollider box = parent.transform.Find("GroundTruthBox3D").transform.GetComponent<BoxCollider>();
                    cent = box.bounds.center;
        ...
        public override void OnVisualize(Visualizer visualizer)
        {
            foreach (var box in Visualized)
            {
                ...
                else if (box.Label == "TrafficLight")
                {
                    color = Color.blue;
                }
