    ...
    public class GroundTruth3DVisualizer : SensorBase
    {
        ...
        public override void OnVisualize(Visualizer visualizer)
        {
            foreach (var detected in Detected)
            {
                ...
                /*
                Vector3 pos = (float3)detected.Position;
                if(detected.Label == "TrafficLight")
                {
                    pos.y = pos.y - detected.Scale.y/2;
                }

                var transform = Matrix4x4.TRS((float3)pos, detected.Rotation, Vector3.one);
                WireframeBoxes.Draw(transform, Vector3.zero, detected.Scale, color);
                */
            }
        }
