...
def performSSDAfter(PATHS, output_path, json_dic, json_path):	 
  with detection_graph.as_default():
    with tf.Session(graph=detection_graph, config=config) as sess:
      for image_path in PATHS:
        ...
        #for each detected box, check if it is in mask
        for i in range(len(boxes[0])):
            if not np.sum(boxes[0][i]) == 0:
                centerY =  int(((boxes[0][i][0] + boxes[0][i][2])/2) * height)
                centerX = int(((boxes[0][i][1] + boxes[0][i][3])/2) * width)
                if mask[centerY][centerX] == 1:
                    new_boxes[0].append(boxes[0][i])
                    new_classes[0].append(classes[0][i])
                    new_scores[0].append(scores[0][i])
                    new_num_detections[0] += 1.
        
        new_boxes[0].append([0,0,0,0])
        new_boxes[0].append([0,0,0,0])
        new_classes[0].append(0)
        new_classes[0].append(0)
        new_scores[0].append(0)
        new_scores[0].append(0)
        
        boxes = np.asarray(new_boxes)
        scores = np.asarray(new_scores)
        classes = np.asarray(new_classes)
...
'''
performSSDAfter(IMAGE_PATHS, SSD_PATH, json_obj_after, SSD_PATH+'dataAfterMask.json')
