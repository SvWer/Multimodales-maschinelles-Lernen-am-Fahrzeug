  ...
  with detection_graph.as_default():
    with tf.Session(graph=detection_graph, config=config) as sess:
      for image_path in PATHS:
        ...
        image_np = perfomMask(image_np, image_path, [255,255,255])
        ...
        while (i < len(np.squeeze(scores))):
           ...
           if currentScore >= 0.01:
               ...
               cv2.imwrite(output_path + 'with_mask/white_'+imgNumber+'.jpg', im_rgb);				
           i += 1
...
'''
performSSDMask(IMAGE_PATHS, SSD_PATH, json_obj_before, SSD_PATH+'dataMaskWhite.json')
